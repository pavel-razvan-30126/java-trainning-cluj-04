...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Studios in which plays the actor with name 'actor cu 2 oscaruri' are: ");
        for (int i = 0; i < studioDatabase.length; i++) {
        ...
        System.out.println("Movies in which plays at least an actor with age above 50 are: ");
        for (int i = 0; i < studioDatabase.length; i++) {
            for (int j = 0; j < studioDatabase[i].getFilme().length; j++)
                for (int k = 0; k < studioDatabase[i].getFilme()[j].getActori().length; k++) {
                    if (studioDatabase[i].getFilme()[j].getActori()[k].getVarsta() >= 50) {
                        System.out.println(studioDatabase[i].getFilme()[j].getNume());
                        if(j<studioDatabase[i].getFilme().length-1){ //Daca se gaseste un actor cu varsta>50, se trece la urm film
                            j++;
                            k=0;
                        }
                    }
